...
def main() -> None:
    ...
    rx_results = send_bundle(alice, bob, bundle_ab)
    print("Receive results:", rx_results)
    print_wallet_state("Alice", alice)
    print_wallet_state("Bob", bob)
    ...
    rx_results = send_bundle(bob, carol, bundle_bc)
    print("Receive results:", rx_results)
    print_wallet_state("Bob", bob)
    ...
    rx_results = send_bundle(carol, dave, bundle_cd)
    print("Receive results:", rx_results)
    print_wallet_state("Carol", carol)
    print_wallet_state("Dave", dave)

    bundle_de = dave.create_payment(receiver_pk=eve_pk, amount=10)
    print(f"\n7) Dave -> Eve at max hop bundle {bundle_de.bundle_id}")
    print(f"Transfers (hop_count): {[(t.transfer_id[:8], t.hop_count) for t in bundle_de.transfers]}")
    rx_results = send_bundle(dave, eve, bundle_de)
    print("Receive results:", rx_results)
    print_wallet_state("Dave", dave)
    print_wallet_state("Eve", eve)

    print("\n8) Wallet overflow attempt (expect REJECTION):")
    frank.receive_token(issuer.mint_token(owner_pk=frank_pk, value=20))
    overflow_bundle = alice.create_payment(receiver_pk=frank_pk, amount=40)
    overflow_result = send_bundle(alice, frank, overflow_bundle)
    print("Receive results:", overflow_result, "(policy MAX_BALANCE rejection expected)")

    reconciler = ReconciliationServer(issuer, policy=policy)
    print("\n9) Reconciliation results:")
    all_bundles = [bundle_ab, bundle_bc, bundle_cd, bundle_de, overflow_bundle]
    ...
    print("\n10) Re-submit first bundle to demonstrate DOUBLE_SPEND:")
